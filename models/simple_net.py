...
class SimpleNet(nn.Module):
    def __init__(self, num_classes=2, drop_rate=0, attnvis = False):
        ...
        self.rgb_base = nn.Sequential(*list(resnet18(pretrained=True).children())[:-2])
        # self.rgb_base = nn.Sequential(*list(resnet18(pretrained=True).children())[:-2])
        ...
        self.lstm = nn.LSTM(input_size=512, hidden_size=100, batch_first=True) # resnet 18
        # self.lstm = nn.LSTM(input_size=2048, hidden_size=256, batch_first=True) # resnet 101
        self.lstm_paper = nn.LSTM(input_size=4608, hidden_size=256, batch_first=True) # paper

        self.last_linear = nn.Linear(100, 1, bias=True)
