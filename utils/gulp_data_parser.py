...
class GulpDataset(object):
    ...
    def __init__(self, input, labels = None, csv = False):
        if csv:
            self.csv_data = self.read_csv_input(input)
        else:
            self.csv_data = self.read_df_input(input)
        
        self.classes = ['Nonexpert', 'Expert']
        self.classes_dict = self.get_two_way_dict(self.classes)

    def read_df_input(self, input_df):
        csv_data = []
        for row in  input_df.itertuples():
            item = ListDataGulp(row[1], row[2])
            csv_data.append(item)
        return csv_data
