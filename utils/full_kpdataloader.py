...
class FullKeyPointDataset(data.Dataset):

    def __init__(self, input, nclips, clip_size, step_size, is_val, transform=None):
        """
        Parameters:
...
#         self.merged_meta_dict = self.gulp_directory.merged_meta_dict
        ...
        self.ANNO_COLS = ['frame_number', 'tool_type', 'p1x', 'p1y', 'p2x', 'p2y', 'p3x', 'p3y', 'p4x', 'p4y', 'p5x', 'p5y', 'p6x', 'p6y']
        self.traject_dir = "./data/kristen_processed/finalized/corrected_done/vid_{:03d}.txt"
    ...
    def _load_vid_traj_df_dic(self):
        ...
        for i in range(num_videos):
            ...
            if not os.path.exists(self.traject_dir.format(vid)):
                ...
                continue
            df = pd.read_csv(self.traject_dir.format(vid), header=None, index_col=0, names=self.ANNO_COLS)
            ...
            #         print((np.array(inter_idx,dtype=np.int64)).dtype)
                        inter_head = np.stack((np.array(inter_idx,dtype=np.int64),np.tile(cur_type,(len(inter_idx)))),axis=1)
                        ...
                        for p in range(12):
                            ...
                            interpred[:,p]=np.interp(inter_idx,data_idx,df.loc[data_idx,anno_pos_col])
                        interpreted = np.concatenate((inter_head,interpred),axis=1)
                        # print(interpreted.shape)
                        # print(self.ANNO_COLS)
                        # print(interpreted)
                        # print("VID: ", vid)
                        append_df = pd.DataFrame.from_records(interpreted,index='frame_number',columns=self.ANNO_COLS)
                        append_df.index = append_df.index.astype(int)
                        for p in range(12):
                            anno_pos_col = self.ANNO_COLS[2+p]
                            append_df[anno_pos_col] = append_df[anno_pos_col].astype(float)
            ...
            print("vid: ", vid, "len(self.vid_traj_df_dic[path])=",len(test_df))
    
    def _gaussian(self, x, mean, val):
        return 1/(val*np.sqrt(2*np.pi))*np.exp(-0.5*np.square((x-mean)/val))
        ...
        for i in range(N):
            for j in range(J):
                ...
                if x<0 or y<0:
                    ...
                    for k in range(scale, -1, -1): #[scale, scale-1, scale-2,...,0]
                        x = int(x)
                        y = int(y)
                        x1 = max(x-k,0)
                        x2 = min(x+k,w)
                        y1 = max(y-k,0)
                        y2 = min(y+k,h)
                        traj_map[i,y1:y2+1,x1:x2+1] = 1-(float(k)/scale)
        return traj_map
    ...
    def _transform(self, data, traj):
        ...
        if self.transform:
            ...
            for i in range(F):
                if i==0:
                    transformed = self.transform(image=data[i],keypoints=traj[i].tolist())
                    replay = transformed['replay']
                    data[i] = transformed['image']
                    C,H,W = data[i].shape
                    new_data = torch.zeros((F,C,H,W))
                else:
                    transformed = A.ReplayCompose.replay(replay,image=data[i],keypoints=traj[i].tolist())
                    data[i] = transformed['image']
            ...
            traj[:,:,0] = np.where(traj[:, :, 0]<0, np.ones_like(traj[:,:,0])*-1, traj[:,:,0])
            traj[:,:,1] = np.where(traj[:, :, 1]<0, np.ones_like(traj[:,:,1])*-1, traj[:,:,1])
            traj[:,:,0] = np.where(traj[:, :, 0]>W, np.ones_like(traj[:,:,0])*-1, traj[:,:,0])
            traj[:,:,1] = np.where(traj[:, :, 1]>H, np.ones_like(traj[:,:,1])*-1, traj[:,:,1])

            traj[:,:,0] = np.where(traj[:, :, 1]<0, np.ones_like(traj[:,:,0])*-1, traj[:,:,0])
            traj[:,:,1] = np.where(traj[:, :, 0]<0, np.ones_like(traj[:,:,1])*-1, traj[:,:,1])
    ...
    def __getitem__(self, index, test_pos=-1):
        ...
        for i in range(self.nclips):
            ...
            slice_object = slice(
                offset, num_frames_necessary//self.nclips + offset, self.step_size)
    ...
    #         frames, meta = self.gulp_directory[item.id, slice_object]
            ...
            if len(frames) < (self.clip_size):
                ori_len = len(frames)
                frames.extend([frames[-1]] *
                            ((self.clip_size) - len(frames)))
                clip_indices = np.concatenate((clip_indices,np.repeat(clip_indices[-1],(self.clip_size-ori_len))))
        ...
        traj_map = self._traj_to_map(traj_data, data[0],7,1)

        return (data, target_idx, traj_map, item_path, frames, indices, traj_data)
...
class KPVideoFold(KeyPointDataset):
    def __init__(self, vid_idx, input, nclips, clip_size, step_size, is_val, transform=None):
        super(KPVideoFold, self).__init__(input,nclips,clip_size,step_size,is_val,transform)
...
def get_img_test_data_loader(config, vid_idx, val_fold=1, test_fold=5, imsize=256, batch_size=512, clip_size=18, 
                        nclips=1, step_size=2, num_workers=0, dum=False, num_folds=5):
